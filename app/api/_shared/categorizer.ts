...
async function embedBatch(texts: string[]): Promise<number[][]> {
  ...
  console.log(`[autoCategory] embedBatch model=${EMBEDDING_MODEL} size=${texts.length}`);
  ...
  if (!res.ok) {
    ...
    console.error(`[autoCategory] embeddings http_error status=${res.status} body=${t.slice(0,200)}`);
...
async function ensureCategoryEmbeddings(
  uid: string,
  cats: Array<{ id: string; name: string; nameLower?: string; embedding?: number[]; embeddingModel?: string }>,
): Promise<Array<{ id: string; name: string; nameLower: string; embedding: number[] }>> {
  // カテゴリ名の埋め込みを作成/更新し、Firestoreにキャッシュ
  const toCompute: { idx: number; text: string }[] = [];
  const out: Array<{ id: string; name: string; nameLower: string; embedding: number[] }> = cats.map((c) => ({
    id: c.id,
    name: c.name,
  ...
  for (let i = 0; i < cats.length; i++) {
    const c = cats[i];
    if (!c.embedding || c.embedding.length === 0 || c.embeddingModel !== EMBEDDING_MODEL) {
      toCompute.push({ idx: i, text: c.name });
    }
  }
  if (toCompute.length > 0) {
    const embeds = await embedBatch(toCompute.map((t) => t.text));
    const batch: Array<{ id: string; embedding: number[]; embeddingModel: string }> = [];
    for (let k = 0; k < toCompute.length; k++) {
      const { idx } = toCompute[k];
      out[idx].embedding = embeds[k];
      batch.push({ id: cats[idx].id, embedding: embeds[k], embeddingModel: EMBEDDING_MODEL });
    }
    // Persist embeddings back to category docs (best-effort)
    await Promise.all(
      batch.map((b) => categoriesRepo.updateEmbedding(uid, b.id, b.embedding, b.embeddingModel).catch(() => void 0)),
    );
  }
  return out;
}
...
async function llmChooseCategory(
  ...
  console.log(`[autoCategory] llm request model=${CHAT_MODEL} cats=${cats.length} textLen=${text.length}`);
  const system = "You are a strict classifier. Choose the best category id given the input text. If none fits, return null.";
  ...
  if (res && res.ok) {
    ...
    try {
      ...
      console.error(`[autoCategory] llm parse_error content=${content.slice(0,200)}`);
    ...
    console.error(`[autoCategory] llm http_error status=${status} body=${body.slice(0,200)}`);
  ...
  try {
    ...
    const prompt = `Given categories (id,name) and text, respond ONLY with compact JSON: {"id": <string|null>, "confidence": <number 0..1>}. Pick one id or null.\n` +
    ...
    if (!gRes.ok) {
      ...
      console.error(`[autoCategory] gemini http_error status=${gRes.status} body=${t.slice(0,200)}`);
      return { categoryId: null, confidence: 0 };
    }
    const gJson: any = await gRes.json();
    const textOut: string | undefined = gJson.candidates?.[0]?.content?.parts?.[0]?.text;
    ...
    try {
      ...
      console.error(`[autoCategory] gemini parse_error content=${(textOut||'').slice(0,200)}`);
      return { categoryId: null, confidence: 0 };
    }
  } catch (e) {
...
export async function autoAssignCategory(
  uid: string,
  link: { id: string; url: string; title?: string | null; description?: string | null },
): Promise<AutoCategoryResult> {
  // 自動分類のエントリポイント
  if (!isEnabled()) return { categoryId: null, confidence: 0, method: "none" };
  const apiKey = getApiKey();
  // OpenAIキーもGeminiキーも無ければ処理不可
  if (!apiKey && !config.geminiApiKey) return { categoryId: null, confidence: 0, method: "none" };
  console.log(
  ...
  if (catDocs.length === 0) return { categoryId: null, confidence: 0, method: "none" };
  ...
  if (config.autoCategoryMode === 'llm_only') {
    ...
    if (llmEnabled()) {
      const llm = await llmChooseCategory(
        text,
        (await categoriesRepo.listRaw(uid)).map((d) => ({ id: d.id, name: (d.data() as any)?.name ?? '' })),
      );
      ...
      if (chosen) {
        ...
        await db
          ...
          .catch(() => void 0);
        return { categoryId: chosen, confidence: llm.confidence ?? 0.5, method: 'llm' };
      }
    }
    console.log(`[autoCategory] none uid=${uid} link=${link.id} reason=llm_only_no_choice`);
    return { categoryId: null, confidence: 0, method: 'none' };
  }
  ...
  try {
    ...
    ensuredCats = cats.map((c) => ({ id: c.id, name: c.name, nameLower: c.nameLower!, embedding: [] }));
  }
  ...
  if (linkVec.length > 0) {
    ...
    if (best.id && best.score >= threshold()) {
      ...
      await db
        ...
        .catch(() => void 0);
      return { categoryId: best.id, confidence: best.score, method: "embedding" };
    }
  }
