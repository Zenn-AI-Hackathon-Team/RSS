...
export async function fetchOGP(
  ...
  try {
    ...
    const isHtml = ctype.includes("text/html") || ctype.includes("application/xhtml+xml");
    ...
    const description = pickFirst(idx, ["og:description", "description", "twitter:description"]);
    const rawImage = pickFirst(idx, ["og:image", "twitter:image", "twitter:image:src"]);
