	...
	return (
		...
		>
			<DialogContent className="flex flex-col w-[640px] max-w-[92vw] h-[560px]">
				...
				<div className="flex-1 space-y-8 py-4 px-1 overflow-auto">
					{step === 0 && (
						<section>
							<h3 className="mb-3 font-semibold">1. カテゴリを作成しよう ✨</h3>
				...
				{/* step indicator under the image, above buttons */}
				<div className="mt-2 mb-2 flex items-center justify-center gap-2">
					{[0, 1, 2].map((i) => (
						<span
							key={i}
							className={`h-2.5 w-2.5 rounded-full ${step === i ? "bg-indigo-600" : "bg-slate-300"}`}
						/>
					))}
				</div>

				<DialogFooter className="flex flex-row justify-between sm:justify-between items-center mt-auto pt-2 w-full">
