...
// --- 検索結果のアイテム ---
const SearchResultItem = ({ post, onClick }: { post: Post, onClick: () => void }) => (
    <div
        onClick={onClick}
        className="flex items-center p-2 space-x-4 rounded-lg cursor-pointer transition-colors hover:bg-slate-50"
    >
        <img
            src={post.thumbnail}
            alt={post.title}
...
// --- 検索ボックス本体 ---
export const SearchBox = ({ posts, categories, onResultClick }: SearchBoxProps) => {
    ...
    const searchResults = useMemo(() => {
        ...
        return posts.filter(post => {
            ...
            const postText = `${post.title} ${category ? category.name : ''} ${post.tags.join(' ')}`.toLowerCase();
    ...
    return (
        <>
            ...
            <div
                ...
                className="relative flex h-10 w-full items-center rounded-md border border-input bg-slate-100 px-3 py-2 text-sm text-slate-500 ring-offset-background cursor-pointer"
            >
                        ...
                        {query && searchResults.length > 0 ? (
                             <div className="space-y-1">
                                {searchResults.map(post => (
                                    <SearchResultItem key={post.id} post={post} onClick={() => handleLinkClick(post.id)} />
                                ))}
                            </div>
                        ) : (
                            query && <p className="text-center text-slate-500 pt-8">検索結果はありません。</p>
                        )}
                    </div>
                    
                    <DialogFooter>
                        <Button variant="outline" onClick={() => handleOpenChange(false)}>キャンセル</Button>
                        <Button type="submit" onClick={() => handleOpenChange(false)}>検索</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </>
